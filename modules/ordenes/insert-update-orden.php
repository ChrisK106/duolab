...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if ($ord_id == "" || $ord_id == null) {
        ...
        $LSTMAXID = $pdo->prepare("SELECT MAX(id) AS MAXID FROM tbl_order WHERE type=" . $ord_tipo . " ORDER BY id DESC");
        $LSTMAXID->execute();
        $NEXT_ID = 0;
        ...
        $sqlStatement = $pdo->prepare("INSERT INTO tbl_order (type,number,status,currency,issue_date,delivery_date,provider_id,payment_days,account_number,quotation,requester,approver,observation,total_purchase,total_tax,total_net,exchange_rate_sale,exchange_rate_purchase) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        $sqlStatement = $pdo->prepare("UPDATE tbl_order SET type=?,status=?,currency=?,issue_date=?,delivery_date=?,provider_id=?,payment_days=?,account_number=?,quotation=?,requester=?,approver=?,observation=?,total_purchase=?,total_tax=?, total_net=?, exchange_rate_sale=?, exchange_rate_purchase=? WHERE id=?");
        if ($sqlStatement) {
            $sqlStatement->execute([$ord_tipo, $ord_estado, $ord_tipomon, $ord_emidate, $ord_deldate, $ord_provider, $ord_daypay, $ord_nrocuenta, $ord_cotiz, $ord_request, $ord_approver, $ord_observ, $ord_totcompra, $ord_totigv, $ord_totneto, $ord_rventa, $ord_rcompra, $ord_id]);
            echo "OK_UPDATE";
        } else {
            echo "ERROR";
        }
    ...
    if ($ord_lstprods != "") {
        if (count($ord_lstprods) > 0) {
            foreach ($ord_lstprods as $key => $value) {
                ...
                $sqlStatement = $pdo->prepare("INSERT INTO tbl_order_detail(order_id,item_code,item_description,item_gloss,item_unit_value,item_unit_price,item_quantity,item_discount_rate,item_discounted_total) VALUES(?,?,?,?,?,?,?,?,?)");
    ...
    if ($ord_lstservicios != "") {
        if (count($ord_lstservicios) > 0) {
            foreach ($ord_lstservicios as $key => $value) {
                ...
                $sqlStatement = $pdo->prepare("INSERT INTO tbl_order_detail(order_id,item_description,item_gloss,item_unit_price,item_quantity,item_discount_rate,item_discounted_total) VALUES(?,?,?,?,?,?,?)");
